...
class QCalendarPicker: UIView, UIGestureRecognizerDelegate, CalendarViewDelegate, TimePickerViewDelegate {
    ...
    open var selectDate: String! {
        set {
            if selectDate != nil && selectDate.count > 0 {
                if let date = newValue.toDate(formatter: "YYYY-MM-dd HH:mm") {
                    updateDateTimeLabel(date: date)
                    
                    calendarView.selectedDay = date
                    calendarView.collectionView.reloadData()
                    
                    timePickerView.datePicker.date = date
                }
            }
        }
        get {
    ...
    init(selectedDate: @escaping DidSelectedDate) {
        ...
        dateBtn.setTitleColor(.darkGray, for: .selected)
        dateBtn.setTitleColor(.gray, for: .normal)
        dateBtn.isSelected = true
        dateBtn.addTarget(self, action: #selector(clickTimeBtn(_:)), for: .touchUpInside)
        ...
        timeBtn.setTitleColor(.darkGray, for: .selected)
        timeBtn.setTitleColor(.gray, for: .normal)
        timeBtn.isSelected = false
        timeBtn.addTarget(self, action: #selector(clickTimeBtn(_:)), for: .touchUpInside)
    ...
    private func updateDateTimeLabel(date: Date) {
        ...
        dateBtn.setTitle("\(year)年\(month)月\(day)日", for: .normal)
        timeBtn.setTitle(currentTime, for: .normal)
    }
    ...
    func didSelectedDate(selecteDate: Date) {
        ...
        dateBtn.setTitle("\(selectedYear)年\(selectedMonth)月\(selectedDay)日", for: .normal)
...
class CalendarCell: UICollectionViewCell {
    ...
    private var _textTemp: String!
    private var _textColorTemp: UIColor!
    
    open var text: String! {
        set {
            _textTemp = newValue
            self.textLbl.text = _textTemp
        }
        get {
            return _textTemp
        }
    ...
    open var textColor: UIColor! {
        set {
            _textColorTemp = newValue
            self.textLbl.textColor = _textColorTemp
        }
        get {
            return _textColorTemp
        }
    }
