...
private let isIPhoneX: Bool = {
    ...
    if #available(iOS 11.0, *) {
        ...
        if mainWindow != nil, mainWindow! != nil, mainWindow!!.safeAreaInsets.bottom > CGFloat(0.0) {
            return true
        }
    }
    return false;
...
class QDatePicker: UIView, UIGestureRecognizerDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    /// 当前日期，需要先设置datePickerStyle
    open var currentDateStr: String {
        set {
            if let date = newValue.toDate(formatter: formatter()) {
                intentDate = date
            }
        }
        get {
            return intentDate.formatterDate(formatter: formatter())
        }
    }
    ...
    init(selectedDate: @escaping DidSelectedDate) {
        ...
        sureView.backgroundColor = themeColor
        sureView.addTarget(self, action: #selector(sureDate(_:)), for: .touchUpInside)
        addSubview(sureView)
    ...
    @objc private func formatter() -> String {
    ...
    @objc private func sureDate(_ button: UIButton) {
        var string = ""
        if pickerStyle == .datePicker {
            string = intentDate.formatterDate(formatter: formatter())
        }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        if pickerStyle == .datePicker {
            ...
            if let date = dateString.toDate(formatter: formatter()) {
                intentDate = date
            }
    ...
    func initDefaultDate() {
        ...
        let year = comps.year
        if maxLimitDate == nil {
            maxLimitDate = String(format: "%ld-12-31 23:59", year! + 10).toDate(formatter: formatter())
        }
        if minLimitDate == nil {
            minLimitDate = String(format: "%ld-01-01 00:00", year! - 10).toDate(formatter: formatter())
        }
    }
